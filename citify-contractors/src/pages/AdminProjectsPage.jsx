...
const toProject = (form, fallbackImage = '', fallbackImages = []) => {
  ...
  return ({
    ...
    details: isLandListing ? '' : form.details,
...
const AdminProjectsPage = () => {
    ...
    setForm((prev) => {
      if (name === 'listingType') {
        ...
        return {
          ...
          plotOptions: nextType === 'land' ? prev.plotOptions : [{ size: '', price: '' }],
        }
      }
      ...
      if (name === 'landPlotMode') {
        return {
          ...
          plotOptions: value === 'multiple'
            ? (prev.plotOptions.length ? prev.plotOptions : [{ size: '', price: '' }])
            : prev.plotOptions,
        }
      }
      ...
      return {
        ...prev,
        [name]: name === 'price' ? formatNaira(value) : value,
      }
    })
    if (name === 'listingType') {
      setFeatureInput('')
  ...
  const addPlotOptionRow = () => {
    setForm((prev) => ({
      ...prev,
      plotOptions: [...prev.plotOptions, { size: '', price: '' }],
    }))
  }
                      ...
                      {form.listingType === 'land' && form.landPlotMode === 'multiple' ? (
                        <div className='rounded-xl border border-slate-200 p-3'>
                          <div className='mb-2 flex items-center justify-between'>
                            <label className='text-xs font-medium text-slate-600'>Plot Sizes and Prices</label>
                            <button type='button' onClick={addPlotOptionRow} className='text-xs px-2.5 py-1 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors'>Add Row</button>
                          </div>
                          <div className='space-y-2'>
                            {form.plotOptions.map((option, index) => (
                              <div key={`plot-option-${index}`} className='grid grid-cols-[1fr_1fr_auto] gap-2'>
                                <input
                                  value={option.size}
                                  onChange={(event) => updatePlotOption(index, 'size', event.target.value)}
                                  placeholder='e.g. 300 sqm'
                                  className='w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#058F44] focus:ring-2 focus:ring-[#058F44]/10 transition-all'
                                />
                                <input
                                  value={option.price}
                                  ...
                                  className='w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#058F44] focus:ring-2 focus:ring-[#058F44]/10 transition-all'
                                />
                                <button type='button' onClick={() => removePlotOptionRow(index)} className='px-2.5 py-2 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 transition-colors'>×</button>
                              </div>
                            ))}
                          </div>
                    ...
                    <div className='space-y-4'>
                      {form.listingType !== 'land' ? (
                        ...
                        <div className='rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-600'>
                          Descriptions are not required for land listings.
                        </div>
                      )}
